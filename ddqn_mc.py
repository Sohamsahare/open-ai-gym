...
class Model(nn.Module):
    ...
    def build_model(self):
        ...
        l3 = nn.Linear(self.hidden2, self.action_size, bias = True)
        model = torch.nn.Sequential(
            l1,
            l2,
            l3
        )
        # model.compile(loss = nn.MSELoss(), optimizer = optim.Adam(lr=self.learning_rate))
        return model        
    ...
    def learn( self, experiences):
        ...
        for i in range(self.batch_size):
            ...
            # self.model.fit(states[i],Q_target[i],epochs=1,verbose=0)
